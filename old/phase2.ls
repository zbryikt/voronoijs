...
svgnode3 = document.getElementById \svg3
box3 = svgnode3.getBoundingClientRect!
svg3 = d3.select \#svg3 .attr "viewBox": [0,0,box2.width,box2.height].join(" ")
...
render = ->
  ...
  polygons = polygons.map -> Polygon-intersect omega, it
  #xrange = d3.extent polygons.filter(->!it.boundary).map -> it.cx
  #yrange = d3.extent polygons.filter(->!it.boundary).map -> it.cy
  xrange = [-1.5 1.5]
  yrange = [-1.5 1.5]
  x2scale = d3.scale.linear!domain xrange .range [50,900]
  y2scale = d3.scale.linear!domain yrange .range [500,50]

  svg2.selectAll \path.voronoi .data (polygons or [])
  ...
  /*
  svg2.selectAll \circle.voronoi .data (polygons or [])
...
makep = (cx, cy,r,c) ->
  ...
  for i from 0 to c
    a = (Math.PI * 2 * i / c)
    ret.push {x: cx + Math.cos(a) * r, y: cy + Math.sin(a) * r}
  ret

polygon-test = (x=1,y=1,side1=6,side2=6) ->
  #p1 = [[0,0],[2,0],[2,2],[0,2]].map -> {x: it.0, y: it.1}
  #p2 = [[1,1],[3,1],[3,3],[1,3]].map -> {x: it.0, y: it.1}
  p1 = makep 2, 1.5, 0.5, side1
  p2 = makep x, y, 0.5, side2
  plg = Polygon-intersect p1, p2
  plg.active = true

  svg3.selectAll \path .data [p1, p2, plg]
    .enter!append \path

  svg3.selectAll \path .attr do
    d: -> 
      if !it.length => return ""
      ["M#{ 200 * it.0.x} #{ 200 * it.0.y}"] ++
      ["L#{ 200 * it[i]x} #{ 200 * it[i]y}" for i from 1 til it.length] ++
      ["L#{ 200 * it.0.x} #{ 200 * it.0.y}"].join(" ")
    fill: -> if it.active => "rgba(255,0,0,0.3)" else "rgba(0,0,0,0.1)"
    stroke: ->
      if it.active => "rgba(255,0,0,0.9)" else "rgba(0,0,0,0.5)"
...
sa2 = 0
setInterval ->
  x = cpx + ppr * Math.cos(ppa)
  y = cpy + ppr * Math.sin(ppa)
  polygon-test(
    x
    y
    6 + parseInt(Math.cos(sa1)*4)
    6 + parseInt(Math.sin(sa2)*4)
  )
  ppr := Math.sin(ppar)
  ppar := (ppar + 0.02) 
  ppa := ppa + 0.03
